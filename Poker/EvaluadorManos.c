 ...
 */
static void contar_numeros(Carta* mano, int* resultados) {
    for(int i = 0; i < 7; i++) {
        resultados[mano[i].numero]++;
    }
}

static void contar_palos(Carta* mano, int* resultados) {
    for(int i = 0; i < 7; i++) {
 ...
 */
static Boolean contiene_pareja(Carta* todas) {
    int conteo[14] = {0};
    contar_numeros(todas, conteo);
    for(int i = 1; i <= 13; i++) {
        if(conteo[i] >= 2) {
            return TRUE;
        }
 ...
 */
static Boolean contiene_doble_pareja(Carta* todas) {
    int conteo[14] = {0};
    contar_numeros(todas, conteo);
 ...
 */
static Boolean contiene_trio(Carta* todas) {
    int conteo[14] = {0};
    contar_numeros(todas, conteo);
    for(int i = 1; i <= 13; i++) {
        if(conteo[i] >= 3) {
 ...
 */
static Boolean contiene_escalera(Carta* todas) {
    qsort(todas, 7, sizeof(Carta), comparar_cartas);
    int consecutivas = 1;
    for(int i = 1; i < 7; i++) {
 ...
 */
static Boolean contiene_color(Carta* todas) {
    int conteo_palos[4] = {0};
    contar_palos(todas, conteo_palos);
    for(int i = 0; i < 4; i++) {
        if(conteo_palos[i] >= 5) {
            return TRUE;
        }
 ...
 */
static Boolean contiene_poker(Carta* todas) {
    int conteo[14] = {0};
    contar_numeros(todas, conteo);
    for(int i = 1; i <= 13; i++) {
        if(conteo[i] >= 4) {
 ...
 */
static Boolean contiene_full(Carta* todas) {
    int conteo[14] = {0};
    contar_numeros(todas, conteo);
 ...
 */
static Boolean contiene_escalera_color(Carta* todas) {
 ...
 */
static Boolean contiene_escalera_real(Carta* todas) {
...
static Boolean contiene_carta_alta(Carta* todas) {
...
static void combinar_cartas(Carta* propias, Carta* comunes, Carta* resultado) {
    for(int i = 0; i < 2; i++){
        resultado[i] = propias[i];
    }
    for(int j = 0; j < 5; j++){
        resultado[j + 2] = comunes[j];
    }
}

Boolean mano_contiene_combinacion(Carta* todas, enum Combinacion combinacion) {

    switch(combinacion) {
        case ESCALERA_REAL:
            return contiene_escalera_real(todas);
        case ESCALERA_COLOR:
            return contiene_escalera_color(todas);
        case POKER:
            return contiene_poker(todas);
        case FULL:
            return contiene_full(todas);
        case COLOR:
            return contiene_color(todas);
        case ESCALERA:
            return contiene_escalera(todas);
        case TRIO:
            return contiene_trio(todas);
        case DOBLE_PAREJA:
            return contiene_doble_pareja(todas);
        case PAREJA:
            return contiene_pareja(todas);
        case CARTA_ALTA:
            return contiene_carta_alta(todas);
        default:
            return FALSE;
    }
}

int valor_mano(Carta* todas) {
    for(enum Combinacion combinacion = ESCALERA_REAL; combinacion >= CARTA_ALTA; combinacion--) {
        Boolean contiene_combinacion = mano_contiene_combinacion(todas, combinacion);
 ...
 */
static Boolean desempate_por_cartas(Carta* a_todas, Carta* b_todas) {
    // Ordenar ambas manos de mayor a menor
    qsort(a_todas, 7, sizeof(Carta), comparar_desc);
    qsort(b_todas, 7, sizeof(Carta), comparar_desc);
...
Boolean mano_jugador_a_mejor_mano_jugador_b(PCB* a, PCB* b, Carta* cartas_comunes) {
    ...
    Carta a_todas[7], b_todas[7];
    combinar_cartas(a->cartas, cartas_comunes, a_todas);
    combinar_cartas(b->cartas, cartas_comunes, b_todas);

    int valor_a = valor_mano(a_todas);
    int valor_b = valor_mano(b_todas);
    ...
    return desempate_por_cartas(a_todas, b_todas);
}